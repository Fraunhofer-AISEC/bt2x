...
async fn main(spawner: Spawner) {
    ...
    // Left in, we assume this has the same code complexity as fetching a regular
    let _ = transport
    ...
    // Dummy verification
    let verifying_key = p256::ecdsa::VerifyingKey::from_public_key_der(&[]).unwrap();
    let signature = p256::ecdsa::DerSignature::from_bytes(&[]).unwrap();
    verifying_key.verify_digest(hasher, &signature).unwrap();
